...
print ('get_negatives.py stars galaxies unknown  no normalize OCT26')

hm_lenses = input("How many lenses do you want to generate parameter files for? ")
NEGATIVES_PATH = "/run/media/toyonagar/Lexar/i_band_mag_lim/n_c/n_cutout_"
NEGATIVES_OUT_PATH = '/run/media/toyonagar/Lexar/i_band_mag_lim/n_o/n_'
...
unknown = 'UNKNOWN  '
galaxy ='GALAXY   '
dataframe = pd.read_csv("i_mag.tsv",delimiter=";")

data_dict ={"ra": [], "dec":[], "pubID":[], "obj_name":[], 'i_mag':[] } #each entry is a different row

length = len(dataframe)
for i in range(length):
    if (dataframe.Class[length-i-1]==galaxy or dataframe.Class[length-i-1]==star or 
        dataframe.Class[length-i-1]==unknown) :
        try:
            if float(dataframe.Imag[length-i-1])<=25.0 or dataframe.Class[length-i-1]==galaxy:               
                data_dict["ra"].append(dataframe["RAJ2000"][length-i-1])
                data_dict["dec"].append(dataframe['DEJ2000'][length-i-1])
                data_dict["i_mag"].append(dataframe['Imag'][length-i-1])
        except:
            pass
