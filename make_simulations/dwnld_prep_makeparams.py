...
CUTOUT_PATH = "/run/media/toyonagar/Lexar/i_band/cutout/cutout_"
...
'''
requires temp_parameters.parorig in same directory because this file contains the flags that this script searches for.
...
for i in range(hm_lenses):   
    ...
    ra.append(float(random.randint(0,70))/100000.)#70 # 50 oct22
    dec.append(-float(random.randint(0,70))/100000.)#70 #50 oct 22
    ...
    redshift.append( float(zlens[i])+ float(random.randint(5,50))/100.) #30-80 oct22
    count =0
    while(redshift[i]>6 and count<10):
        count+=1
        print("trying to get source redshift under 8")
        redshift[i] = float(zlens[i])+ float(random.randint(5,20))/100. #larger makes smaller rings
