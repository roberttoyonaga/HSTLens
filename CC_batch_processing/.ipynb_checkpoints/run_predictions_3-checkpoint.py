...
from HSTLens_resnet16_s_multiclass import deeplens_classifier
import pandas as pd
import os
...
WEIGHTS_PATH = "/home/toyonaga/projects/def-sfabbro/toyonaga/HSTLens/weights/combined_nonsubtracted_weights_resnet16_s_h4"
...
start= time.time()
crd_dict = {'obj_name':[],'ra': [], 'dec': [], 'prb_1':[],'prb_2':[],'prb_3':[],'prb_4':[]}
...
for batch in range(hm_batches+1):
    ...
    '''Feed cutouts to model and add to list of predictions'''
    predictions.append(my_model._predict( ims, y= None, discrete = False))

    

predictions =np.concatenate(predictions) #a 2D array of shape(len, #classes)
print(len(predictions))        
out = pd.DataFrame(crd_dict)
print("out csv len"+str(len(out))+'predictions len' +str(len(predictions)))

for prediction in range(len(predictions)):
    out['prb_1'][prediction] = predictions[prediction][0]
    out['prb_2'][prediction] = predictions[prediction][1]
    out['prb_3'][prediction] = predictions[prediction][2]
    out['prb_4'][prediction] = predictions[prediction][3]
