...
class WeightsSaver(Callback):
    ...
    def on_epoch_end(self, batch, logs={}):
        if self.epoch % self.N == 0:
            name = 'epoch_weights%08d.h5' % self.epoch
...
#         out=Dropout(0.30)(out) 
    ...
    #def _fit(self,data,labels,val):
        ...
        callbacks_list = [lrate, self.batch_history, EarlyStopping(monitor='val_loss', patience=30), WeightsSaver(1)]
...
#                        shuffle= True, validation_split = 0.5, epochs= self.n_epochs)
        ...
        self.model.fit(data, labels,validation_data =val, batch_size=self.batch_size, callbacks=callbacks_list,
                       shuffle= False, validation_split = 0, epochs= self.n_epochs)        
        self.fitted=True
