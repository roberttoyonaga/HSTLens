...
WEIGHTS_PATH = "/home/toyonaga/projects/def-sfabbro/toyonaga/HSTLens/CC_batch_processing/batchjob_3/batchjob3_weights/" #ensembleweights_"
...
for prediction in range(len(predictions[0])):
    ...
    crd_dict['prb_1'][prediction] = np.average(np.array([predictions[0][prediction][0], predictions[1][prediction][0], predictions[2][prediction][0]]))
    
    crd_dict['prb_2'][prediction] = np.average(np.array([predictions[0][prediction][1], predictions[1][prediction][1], predictions[2][prediction][1]]))
    
    crd_dict['prb_3'][prediction] = np.average(np.array([predictions[0][prediction][2], predictions[1][prediction][2], predictions[2][prediction][2]]))

out = pd.DataFrame(crd_dict) #after calcs to improve speed
